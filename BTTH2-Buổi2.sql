...
--Phần 3
-- 1) Dùng UNION hiển thị thông tin ModifiedDate của Person.Person và HireDate
--của HumanResources.Employee.
SELECT ModifiedDate FROM Person.Person
UNION
SELECT HireDate FROM HumanResources.Employee;
--2) Dùng CTE tạo một query hiển thị thông tin tổng tiền của mỗi nhân viên bán hàng
--SalePerson. Thông tin gồm FirstName + ' ' + LastName AS FullName, City + ',
--‘+ StateProvinceName AS Location, NetSales, với NetSales=sum(SubTotal)
--được tạo từ CTE gồm 2 field SalesPersonID, NetSales
WITH NetSalesCTE AS
(
    SELECT SalesPersonID, SUM(SubTotal) AS NetSales
    FROM Sales.SalesOrderHeader
    GROUP BY SalesPersonID
)
SELECT p.FirstName + ' ' + p.LastName AS FullName,
       a.City + ', ' + stp.Name AS Location,
       n.NetSales
...
INNER JOIN Person.StateProvince stp on a.StateProvinceID =stp.StateProvinceID
INNER JOIN NetSalesCTE n ON n.SalesPersonID = sp.BusinessEntityID;
...
--c)
DELETE FROM MyOrder 
WHERE PurchaseOrderID NOT IN (1,2);
...
--e)
MERGE INTO MyOrder AS TARGET 
USING Purchasing.PurchaseOrderHeader AS SOURCE 
ON (TARGET.PurchaseOrderID = SOURCE.PurchaseOrderID) 
WHEN MATCHED THEN 
    UPDATE SET TARGET.TotalDue = SOURCE.TotalDue 
WHEN NOT MATCHED THEN 
    INSERT (PurchaseOrderID, EmployeeID, VendorID, ShipMethodID, TotalDue, RevisionNumber, Status) 
    VALUES (SOURCE.PurchaseOrderID, SOURCE.EmployeeID, SOURCE.VendorID, SOURCE.ShipMethodID, SOURCE.TotalDue, SOURCE.RevisionNumber, SOURCE.Status);